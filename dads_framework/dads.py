...
def algorithm_DSL(model, model_input, edge_latency_list, cloud_latency_list, bandwidth, net_type="wifi"):
    ...
    :return: 有向图中的对应的割集（不包含edge顶点和cloud顶点）以及划分过程会用到的 dict_node_layer，记录了顶点对应了模型的第几层
    """
    # 构建模型对应的有向图
    graph, dict_node_layer, dict_layer_input_size = graph_construct(model, model_input, edge_latency_list,
                                                                    cloud_latency_list, bandwidth=bandwidth,
                                                                    net_type=net_type)
    # min_cut_value表示最小割的割数，reachable表示需要放在边缘端推理的顶点， non_reachable表示放在云端推理的顶点
...
def get_partition_points(graph_partition_edge, dict_node_layer):
    """
    根据有向图的割集graph_partition_edge转换成DNN模型切分点model_partition_edge
    ...
    for graph_edge in graph_partition_edge:
        # 表示在DNN模型中的第start_layer层到end_layer之间进行划分(也就是说在start_layer之后进行划分)

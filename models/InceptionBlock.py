...
def construct_edge_cloud_inception_block(model: InceptionBlock, model_partition_edge: list):
    ...
    if len(model_partition_edge) == 1:  # 只有一个地方需要划分
        ...
        for edge in model_partition_edge:
            ...
            if edge[0] in range(accumulate_len[0] + 1, accumulate_len[1] + 1) or edge[1] in range(accumulate_len[0] + 1,
                                                                                                  accumulate_len[
                                                                                                      1] + 1):
                block = branches[0]
                tmp_point = edge[0] - accumulate_len[0]
            elif edge[0] in range(accumulate_len[1] + 1, accumulate_len[2] + 1) or edge[1] in range(
                    accumulate_len[1] + 1, accumulate_len[2] + 1):
                block = branches[1]
                tmp_point = edge[0] - accumulate_len[1]
            elif edge[0] in range(accumulate_len[2] + 1, accumulate_len[3] + 1) or edge[1] in range(
                    accumulate_len[2] + 1, accumulate_len[3] + 1):
                block = branches[2]
                tmp_point = edge[0] - accumulate_len[2]
            elif edge[0] in range(accumulate_len[3] + 1, accumulate_len[4] + 1) or edge[1] in range(
                    accumulate_len[3] + 1, accumulate_len[4] + 1):
                block = branches[3]

...
def get_socket_server(ip, port, max_client_num=10):
    ...
    # 判断使用的是什么平台，针对不同的平台使用不同的socket协议
    ...
    if "windows" in sys_platform:
        ...
        # macos or linux
        # socket.SO_REUSEPORT允许相同主机上的多个socket绑定到同一个端口
        # 用来提升多核系统上多线程网络server应用的性能。
        socket_server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
    # 绑定端口号
    socket_server.bind((ip, port))
    # 打开socket的监听，最多同时允许10个客户端一起连接
    socket_server.listen(max_client_num)
    # 返回用来监听的socket服务
...
def wait_client(p):
    ...
    :return:
    """
    # accept()用于接受一个客户端的连接请求，并返回一个新的套接字
    # 不同于用于监听和接受客户端的连接请求的套接字；与此客户端通信是通过在这个新的套接字上发送和接收数据来完成的
...
def get_short_data(conn):
    """ 获取短数据"""
    # pickle.loads用于将数据从bytes反序列化为原来的值，例如：
    # import pickle
    # l1=[1,2,3,4,5]
    # t1=(1,2,3,4,5)
...
def create_server(p):
    ...
    while True:
        ...
        while True:
            ...
            if len(packet) < 1024:
                break
        # 发送和接收数据都使用pickle包，所以这里进行解析pickle
        parse_data = pickle.loads(b"".join(data))
...
def show_speed(data_size, actual_latency, speed_Bpms):
    ...
    """
    # iperf获取的带宽
    print(f"actual speed : {speed_Bpms:.3f} B/ms")
    # 通过数据大小和真实传输时间计算的带宽
    print(f"predicted speed : {(data_size / actual_latency):.3f} B/ms")
    # 实际记录的时延
    print(f"actual latency for {data_size} bytes : {actual_latency:.3f} ms")
    # 通过iperf带宽预测的时延
    print(f"predicted latency for {data_size} bytes : {(data_size / speed_Bpms):.3f} ms")
